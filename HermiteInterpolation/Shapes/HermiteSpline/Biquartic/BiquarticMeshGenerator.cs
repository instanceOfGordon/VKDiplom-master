...
namespace HermiteInterpolation.Shapes.HermiteSpline.Biquartic
{
    public class BiquarticMeshGenerator : BicubicMeshGenerator
    {
        ...
        public BiquarticMeshGenerator(double uMin, double uMax, int uCount, double vMin, double vMax, int vCount, InterpolatedFunction function,
            IKnotsGenerator knotsGenerator, Derivation derivation)
            : base(uMin, uMax, (uCount+1)/2, vMin, vMax, (vCount+1)/2, knotsGenerator, derivation)
        {
            _interpolatedFunction = function;
