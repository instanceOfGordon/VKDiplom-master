...
namespace HermiteInterpolation.Shapes.HermiteSpline.Biquartic
{
    internal class BiquarticBasis : Basis
    {
        ...
        internal override Matrix<double> Matrix(int uIdx, int vIdx)
        {
            ...
            m[0, 2] = k02.Z; //SafeCall(f, u0, v2);
            m[0, 3] = k00.Dy; //SafeCall(dy, u0, v0);
            m[0, 4] = k02.Dy; //SafeCall(dy, u0, v2);
            m[1, 0] = k10.Z; //SafeCall(f, u1, v0);
            m[1, 1] = k11.Z; //SafeCall(f, u1, v1);
            m[1, 2] = k12.Z; //SafeCall(f, u1, v2);
            m[1, 3] = k10.Dy; //SafeCall(dy, u1, v0);
            m[1, 4] = k12.Dy; //SafeCall(dy, u1, v2);
            m[2, 0] = k20.Z; //SafeCall(f, u2, v0);
            m[2, 1] = k21.Z; //SafeCall(f, u2, v1);
            m[2, 2] = k22.Z; //SafeCall(f, u2, v2);
            m[2, 3] = k20.Dy; //SafeCall(dy, u2, v0);
            m[2, 4] = k22.Dy; //SafeCall(dy, u2, v2);

            m[2, 0] = k00.Dx; //SafeCall(dx, u0, v0);
            m[2, 1] = k01.Dx; //SafeCall(dx, u0, v1); 
            m[2, 2] = k02.Dx; //SafeCall(dx, u0, v2);
            m[2, 3] = k00.Dxy; //SafeCall(dxy, u0, v0);
            m[2, 4] = k02.Dxy; //SafeCall(dxy, u0, v2);
            m[3, 0] = k20.Dx; //SafeCall(dx, u2, v0);
            m[3, 1] = k21.Dx; //SafeCall(dx, u2, v1);
            m[3, 2] = k22.Dx; //SafeCall(dx, u2, v2);
            m[3, 3] = k20.Dxy; //SafeCall(dxy, u2, v0);
            m[3, 4] = k22.Dxy; //SafeCall(dxy, u2, v2);
