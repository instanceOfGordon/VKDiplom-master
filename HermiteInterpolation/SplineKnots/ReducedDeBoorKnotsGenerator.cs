...
namespace HermiteInterpolation.SplineKnots
{
    ...
    {
        public ReducedDeBoorKnotsGenerator(InterpolativeMathFunction function)
            : base(function)
        {
        }
        ...
        {
            if (uDimension.KnotCount < 6
                || vDimension.KnotCount < 6)
            {
                return
                    new DeBoorKnotsGenerator(Function).GenerateKnots(
        ...
        {
            var diag =
                MyArrays.InitalizedArray<double>(
                    unknownsCount%2 == 0 ? unknownsCount/2 - 1 : unknownsCount/2,
                    -14);
        ...
        {
            return
                base.LowerDiagonal(unknownsCount%2 == 0
                    ? unknownsCount/2 - 1
                    : unknownsCount/2);
        }
        ...
        {
            return
                base.UpperDiagonal(unknownsCount%2 == 0
                    ? unknownsCount/2 - 1
                    : unknownsCount/2);
        }
        ...
        protected override double[] RightSide(
            ...
            int unknownsCount)
        {
            ...
            rs[0] = threeDivH*(rightSideVariables(4) - rightSideVariables(0))
                    - twelveDivH*(rightSideVariables(3) - rightSideVariables(1))
                    - dfirst;
            ...
            for (var k = 2; k < equationsCount; k++)
            {
                var k2 = k*2;
                rs[k - 1] = threeDivH
                            *(rightSideVariables(2*(k + 1))
                              - rightSideVariables(2*(k - 1))
                              - twelveDivH
                              *(rightSideVariables(k2 + 1)
                                - rightSideVariables(k2 - 1)));
            }
        ...
        protected override void SolveTridiagonal(
            Func<int, double> rightSideValuesSelector, double h, double dfirst,
            double dlast, int unknownsCount, Action<int, double> unknownsSetter,
            double[] lowerDiagonal = null, double[] mainDiagonal = null,
            double[] upperDiagonal = null)
        {
            if (lowerDiagonal != null
                ...
                && upperDiagonal != null)
            {
                base.SolveTridiagonal(rightSideValuesSelector, h, dfirst, dlast,
                    unknownsCount, unknownsSetter, lowerDiagonal, mainDiagonal,
                    upperDiagonal);
                return;
            }
            lowerDiagonal = lowerDiagonal ?? LowerDiagonal(unknownsCount);
