...
splineknots::ReducedTridiagonal::Solve(size_t num_unknowns)
{
	...
	auto& buffer = Buffer();
	if (tridiagonal_.IsUsingOptimizedTridiagonal())
	{
		utils::SolveDeboorTridiagonalSystemBuffered(MainDiagonalValue(),
			&rightside.front(), num_equations, &buffer.front(), last_maindiag_value);
	}
	else
	{
		utils::SolveCsabaDeboorTridiagonalSystemBuffered(4,
			&rightside.front(), num_unknowns, &buffer.front(),
			&tridiagonal_.csabaTrid_.U.front(),
			&tridiagonal_.csabaTrid_.Y.front(),
			&tridiagonal_.csabaTrid_.D.front());
	}
	return rightside;
}
